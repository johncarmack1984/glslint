...
- `symbols.rs` — a line-based symbol scanner over the assembled unit (UBO/interface blocks, top-level `uniform`/`in`/`out`, function definitions), each symbol carrying its original `Loc`. Powers hover and go-to-definition, including the cross-module jump: `wind.uMin` in `draw.vert.glsl` resolves to its declaration in `windUniforms.glsl`.
- `lsp.rs` — tower-lsp; `publishDiagnostics` on open/change/save, plus hover and go-to-definition, filtered to the edited document. Edits are debounced and the (subprocess-spawning) check runs off the async runtime, with a per-document generation guard so a slow check can't clobber a newer edit.
